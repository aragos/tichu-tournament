...
class MovementTest(unittest.TestCase):
    ...
    for i in range(num_pairs):
      ...
      for round in movement.GetMovement(i + 1):
        ...
        self.assertFalse(opp in opponents_played,
                         msg="Opponent {} for pair {} is played more than once".format(opp, i + 1))
        opponents_played.add(opp)
        
        round_no = round.get('round')
        self.assertIsNotNone(round_no)
        self.assertFalse(round_no in rounds_played,
                         msg="Round {} for pair {} is played more than once".format(round_no, i + 1))
        ...
        for hand in hands:
          self.assertFalse(hand in hands_played, 
                           msg="Hand {} for pair {} is played more than once".format(hand, i + 1))
          hands_played.add(hand)
        position = round.get('position')
        self.assertEqual(2, len(position))
        self.assertTrue(int(position[0]) <= num_pairs/2, msg="Invalid table specification {} for pair {}".format(position, i + 1))
        self.assertTrue(position[1] in ("E", "N"), msg="Invalid position specification {} for pair {}".format(position, i + 1))
      ...
      for round in movement.GetMovement(i + 1):
        ...
        for opp_round in opp_movement:
          ...
          self.assertEqual(hands, opp_round.get('hands'), 
                           msg="Round {} hands for pair {} are inconsistent with opponent pair {}'s hands".format(round_no, i+1, opp))
          self.assertEqual(i + 1, opp_round.get('opponent'),
                           msg="Round {} opponent for pair {} is inconsistent with pair {}".format(round_no, i+1, opp))
          opp_position = opp_round.get('position')
          self.assertEqual(position[0], opp_position[0], msg="Pair {} and pair {} seem to be sitting at different tables for round {}".format(i + 1, opp, round_no))
          self.assertNotEqual(position[1], opp_position[1], msg="Pair {} and pair {} seem to be sitting in the same seat for round {}".format(i + 1, opp, round_no))
        self.assertTrue(found_match,
                        msg="Opponent {} of Pair {} doesn't seem to have them for round {}".format(opp, i+1, round_no))
  
  def checkHandsPlayedRightNumberOfTimes(self, movement, num_pairs, num_hands_per_round):
    # dict of round to hand to num times played
    # relay hands dict of round to hand to num_times played
    non_relay_hands = {}
    relay_hands = {}
    # dict of hand to num
    total_times_played = {}
      ...
      for round in movement.GetMovement(i + 1):
        ...
        if round.get('relay_table'):
          ...
          for hand in hands:
            ...
            non_relay_hands[round_no][hand] = non_relay_hands[round_no].get(hand, 0) + 1
            total_times_played[hand] = total_times_played.get(hand, 0) + 1
    
    for r, hl in non_relay_hands.items():
      for h, n in hl.items():
        self.assertEqual(2, n, msg="Non relay hand {} is played {} times in round {}".format(h, n, round))
    for r, hl in relay_hands.items():
      for h, n in hl.items():
        self.assertTrue(n > 2, msg="Relay hand {} is played only {} times in round {}. Must be > 2.".format(h, n, round))
        self.assertTrue(n % 2 == 0, msg="Relay hand {} is played {} times in round {}. Must be divisible by 2".format(h, n, round))
    first_number_played = total_times_played[1]
    for h, n in total_times_played.items():
      self.assertEqual(first_number_played, n, msg="Hand {} is played {} times, wheras hand 1 is played {} times.".format(h, n, first_number_played))
  ...
  def checkTableConsistency(self, movement, num_pairs, num_hands_per_round):
    ...
    for t, rt in positions.items():
      for r, n in rt.items():
        self.assertEqual(2, len(n), msg="Table {} hosts a weird number of teams ({}) in round {}.".format(t, n, r))
